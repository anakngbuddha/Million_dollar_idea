...
export const ClassroomQuizAnalyticsPage: React.FC = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!classroomQuizId || !classroomId || !user) {
        setError('Invalid classroom quiz or user');
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const classroomQuiz = allClassroomQuizzes.find(q => q.id === classroomQuizId);
        ...
        setQuizTitle(classroomQuiz.quizTitle);

        // Get the custom quiz to verify ownership
        const customQuiz = await getCustomQuiz(classroomQuiz.quizId);
        if (!customQuiz) {
        ...
        const attemptsData = await getAllQuizAttempts(classroomQuiz.quizId);
    ...
    loadData();
  }, [classroomQuizId, classroomId, user]);

  const handleExportToExcel = () => {
    if (!quizTitle || attempts.length === 0) return;
    ...
    XLSX.writeFile(wb, `${quizTitle}-analytics.xlsx`);
  };
