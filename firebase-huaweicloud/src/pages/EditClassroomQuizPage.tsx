...
export const EditClassroomQuizPage: React.FC = () => {
  const { user } = useAuth();
  const navigate = useNavigate();
  const { classroomId, classroomQuizId } = useParams<{ classroomId: string; classroomQuizId: string }>();
  ...
  const [classroomQuizData, setClassroomQuizData] = useState<any>(null);
  const [customQuizData, setCustomQuizData] = useState<CustomQuiz | null>(null);

  // Load quiz data on mount
  useEffect(() => {
    const loadQuiz = async () => {
      if (!classroomQuizId || !classroomId) {
        setError('Invalid quiz or classroom ID');
        setLoading(false);
        return;
      }

      try {
        const allClassroomQuizzes = await getClassroomQuizzes(classroomId);
        const foundQuiz = allClassroomQuizzes.find(q => q.id === classroomQuizId);

        if (!foundQuiz) {
          setError('Classroom quiz not found');
          setLoading(false);
          return;
        }

        setClassroomQuizData(foundQuiz);

        // Load the custom quiz data
        const customQuiz = await getCustomQuiz(foundQuiz.quizId);
        if (!customQuiz) {
          setError('Base quiz not found');
          setLoading(false);
          return;
        }

        // Check if user is the creator
        if (customQuiz.creatorId !== user?.uid) {
          setError('You do not have permission to edit this quiz');
          setLoading(false);
          return;
        }

        setCustomQuizData(customQuiz);
        setQuizTitle(customQuiz.title);
        setQuizDescription(customQuiz.description || '');
        setQuestions(customQuiz.questions || []);
        
        if (foundQuiz.dueDate) {
          const dueDateTime = foundQuiz.dueDate && typeof foundQuiz.dueDate.toDate === 'function'
            ? foundQuiz.dueDate.toDate()
            : foundQuiz.dueDate ? new Date(foundQuiz.dueDate as any) : new Date();
    ...
    loadQuiz();
  }, [classroomQuizId, classroomId, user?.uid]);
  ...
  const handleSaveQuiz = async () => {
    ...
    setSaving(true);
    try {
      if (!customQuizData) throw new Error('Quiz data not found');
      ...
      // Update the custom quiz
      await updateCustomQuiz(customQuizData.id!, updatedQuiz);

      // Update classroom quiz assignment with new due date
      if (classroomQuizData && dueDate) {
        await updateClassroomQuizAssignment(classroomQuizData.id!, {
